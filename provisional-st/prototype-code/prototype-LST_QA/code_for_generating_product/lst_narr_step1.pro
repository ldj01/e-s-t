...
;
; CALL SEQUENCE: lst_narr_step1, home, $            ;directory containing programs and supporting files
...
;##### Read in GRIB data and pull out points pertinent to current Landsat scene
   ...
   ;expand range to include MERRA points outside image for edge pixels
   UL_LAT = UL_LAT + 0.5
   UL_LON = UL_LON - 0.5
   LR_LAT = LR_LAT - 0.5
   LR_LON = LR_LON + 0.5
   ...
   geometricHeight_1 = CONVERT_GEOPOT2GEOMET(landsatHGT_1, lat[NARRindices])
   geometricHeight_1 = geometricHeight_1/1000D
   geometricHeight_2 = CONVERT_GEOPOT2GEOMET(landsatHGT_2, lat[NARRindices])
...
;##### Building tape5 files
   ...
   PRINT, i
      ...
      CASE month OF
         1: JDAY = (day - 1)
         2: JDAY = 31 + (day - 1)
         3: IF leap THEN JDAY = 31 + 29 + (day-1) ELSE JDAY = 31 + 28 + (day-1)
         4: IF leap THEN JDAY = 91 + (day-1) ELSE JDAY = 90 + (day-1)
         5: IF leap THEN JDAY = 121 + (day-1) ELSE JDAY = 120 + (day-1)
         6: IF leap THEN JDAY = 152 + (day-1) ELSE JDAY = 151 + (day-1)
         7: IF leap THEN JDAY = 182 + (day-1) ELSE JDAY = 181 + (day-1)
         8: IF leap THEN JDAY = 213 + (day-1) ELSE JDAY = 212 + (day-1)
         9: IF leap THEN JDAY = 244 + (day-1) ELSE JDAY = 241 + (day-1)
         10: IF leap THEN JDAY = 274 + (day-1) ELSE JDAY = 273 + (day-1)
         11: IF leap THEN JDAY = 305 + (day-1) ELSE JDAY = 304 + (day-1)
         12: IF leap THEN JDAY = 335 + (day-1) ELSE JDAY = 334 + (day-1)
      ENDCASE
