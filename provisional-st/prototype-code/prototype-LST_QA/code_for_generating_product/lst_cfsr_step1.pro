...
PRO lst_cfsr_step1, home, $
  ...
  ;
  p = ['1000', '0975', '0950', '0925', '0900', '0875', '0850', '0825', '0800', '0775', '0750', '0700', '0650', $
       '0600', '0550', '0500', '0450', '0400', '0350', '0300', '0250', '0225', '0200', '0175', '0150', '0125', $
       '0100', '0070', '0050', '0030', '0020', '0010', '0007', '0005', '0003', '0002', '0001']
       
  gridsize= 720L*361L
  hgt_1 = MAKE_ARRAY(gridsize, N_ELEMENTS(p), /DOUBLE)
  hgt_2 = MAKE_ARRAY(gridsize, N_ELEMENTS(p), /DOUBLE) 
  shum_1 = MAKE_ARRAY(gridsize, N_ELEMENTS(p), /DOUBLE)
  shum_2 = MAKE_ARRAY(gridsize, N_ELEMENTS(p), /DOUBLE)
  tmp_1 = MAKE_ARRAY(gridsize, N_ELEMENTS(p), /DOUBLE)
  tmp_2 = MAKE_ARRAY(gridsize, N_ELEMENTS(p), /DOUBLE)
  ...
  FOR i = 0, N_ELEMENTS(p)-1 DO BEGIN
    ...
    hgtFile = directory+'gribfiles/HGT_1/HGT1_'+filepart
    shumFile = directory+'gribfiles/SHUM_1/SHUM1_'+filepart
    tmpFile = directory+'gribfiles/TMP_1/TMP1_'+filepart
  ...
  ;
  ; read in same files for time after Landsat acquisition
  ;
  FOR i = 0, N_ELEMENTS(p)-1 DO BEGIN
    
    filepart = p[i]+'mbar.txt'
    hgtFile = directory+'gribfiles/HGT_2/HGT2_'+filepart
    shumFile = directory+'gribfiles/SHUM_2/SHUM2_'+filepart
    tmpFile = directory+'gribfiles/TMP_2/TMP2_'+filepart
    ...
    CLOSE, 20
    FREE_LUN, 20
    shum_2[*,i] = tempSHUM[2:gridsize+1]
  ...
  ivector = REFORM(ivalues,N_ELEMENTS(ivalues)) ;REFORM(ivalues,1,N_ELEMENTS(ivalues))-1
  jvector = REFORM(jvalues,N_ELEMENTS(jvalues)) ;REFORM(jvalues,1,N_ELEMENTS(jvalues))-1
  
  inLandsatArray = [ivector,jvector]
   
  CFSRindices = MAKE_ARRAY(N_ELEMENTS(ivalues), /DOUBLE)
  FOR i = 0, N_ELEMENTS(CFSRindices)-1 DO BEGIN
     CFSRindices[i] = WHERE(eye EQ ivector[i] AND jay EQ jvector[i])
  ENDFOR
  
 
  numPoints = N_ELEMENTS(CFSRindices) ;N_ELEMENTS(ivector)
  ...
  PRINT, SIZE(landsatHGT_1)  
  ;
  ; select paramaters using indexes of which CFSR points are within the scene
  ;
  
  FOR g = 0, N_ELEMENTS(inLandsat)-1 DO BEGIN
    landsatHGT_1 = hgt_1[CFSRindices,*]   ; hgt_1[inLandsatArray[0,g], inLandsatArray[1,g], *]
    landsatTMP_1 = tmp_1[CFSRindices,*]   ; tmp_1[inLandsatArray[0,g], inLandsatArray[1,g], *]
    landsatSHUM_1 = shum_1[CFSRindices,*] ; sh_1[inLandsatArray[0,g], inLandsatArray[1,g], *]
    landsatHGT_2 = hgt_2[CFSRindices,*]   ; hgt_2[inLandsatArray[0,g], inLandsatArray[1,g], *]
    landsatTMP_2 = tmp_2[CFSRindices,*]   ; tmp_2[inLandsatArray[0,g], inLandsatArray[1,g], *]
    landsatSHUM_2 = shum_2[CFSRindices,*] ; sh_2[inLandsatArray[0,g], inLandsatArray[1,g], *]
  ENDFOR
  ...
  ;
  ;PRINT, 'fillhgt = ',WHERE(ABS(landsatHGT_1 - 1.00000e15) LT 0.0001)
  ;PRINT, 'fillt = ', WHERE(ABS(landsatTMP_1 - 1.00000e15) LT 0.0001)
