...
list of such documented cases:
   ...
   In LSTprocess_merra.bash
      
        line 261: If uncertainty option is set, change path to where dist2cloud images live.
   ...
   In lst_narr_step2.pro
      lst_merra_step2.pro

        line 130: May need to change index value. It is indexing a list of strings between any path
                  separators ('/') from the current path from caseList in order to find lat_lon. 
                  E.g. if the index = 6 it expects that the 6th chunk in the path is lat_lon values.
        
        line 131: May need to change index value. It is indexing a list of strings between any path
                  separators ('/') from the current path from caseList in order to find height. 
                  E.g. if the index = 7 it expects that the 7th chunk in the path contains height.
