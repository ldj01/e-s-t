...
for pathrow in "LSTscenes2run"; do
	...
	for file in "$pathrow/"*_MTL.txt; do
		...
		# "no" to omit generation of uncertainty band, "yes" to include it (requires distance to cloud images)
                generate_uncertainty="yes"
		# Submit the job
		sbatch --qos free --partition work -x woodcrest-[01-24] --mem=8000 -J $jobname -o $outfile $jobfile $base'_merra' $base $generate_uncertainty
	done;
done
