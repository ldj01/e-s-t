...
;
;
; NAME:  lst_merra_step3_zerodem
;  
; PURPOSE:  IDL PROCEDURE
;   Generate transmission, upwelled radiance, and downwelled radiance at each pixel
;
; CALL SEQUENCE: lst_merra_step3_zerodem,  home, $            ;directory containing programs and supporting files
...
PRO lst_merra_step3_zerodem,  home, $
                      directory, $
                      imageBase, $
                      numPoints, $
                      numHeights, $
...
;   offsetNorthing = ROUND(ABS(UL_demNorthing-UL_NORTH)/30D)
   ...
   i = INDGEN(288)+1
   lonarray = -180 + deltalambda*(i-0.5)
   lon = MAKE_ARRAY(288,144, /DOUBLE)
   ...
   j = INDGEN(144)+1
   latarray = -90 + deltaphi*(j-0.5)
   lat = MAKE_ARRAY(288,144, /DOUBLE)
...
;   results[4,*,*] = downwelled 
   ...
   ; Don't think we need to save upperleft info
