...
'''
    File: cfmask_lst_generate_distance_to_cloud.py

    Purpose: Builds a band for distance to cloud based on cfmask input. 
             This is a development tool used to compare prototype results
             which use cfmask to production results which use pixel_qa.
             This tool is the same as the production cloud distance tool,
             except that it uses the prototype's cfmask input instead of
             pixel_qa.
...
def calculate_distance(src_info, fill_value):
    ...
    qa_cloud_locations = np.where(qa_data == 4)
    qa_fill_locations = np.where(qa_data == 255)
    qa_cloud_background = qa_data # just give it the right shape
    qa_cloud_background.fill(1)
    qa_cloud_background[qa_cloud_locations] = 0
